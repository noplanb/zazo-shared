class Event < BaseConnection::EventsDb
  ...
  def self.status_transitions(mkey = nil)
    scope = Event
    scope = scope.by_initiator 'user', mkey if mkey
    scope.name_overlap %w(invited initialized registered verified)
  end

  def self.messages_sent(mkey = nil)
    scope = Event
    scope = scope.with_sender mkey if mkey
    scope.video_s3_uploaded
  end

  def self.invites(mkey = nil)
    scope = Event
    scope = scope.by_initiator 'user', mkey if mkey
    scope.by_name %w(user invitation_sent)
  end
end
